...
exports.getTokenByCode = (oxdServerURL, requestBody, clientToken) => {
  const option = {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${clientToken}`
    },
    uri: `${oxdServerURL}/get-tokens-by-code`,
    body: requestBody,
    resolveWithFullResponse: true,
    json: true
  };
...
exports.getUserInfo = (oxdServerURL, requestBody, clientToken) => {
  const option = {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${clientToken}`
    },
    uri: `${oxdServerURL}/get-user-info`,
    body: requestBody,
    resolveWithFullResponse: true,
    json: true
  };
